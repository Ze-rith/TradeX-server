...
| 모듈 | 레이어 | 증명하는 것 |
|---|---|---|
| `tradex-core` | L0 Bi-temporal Domain Core | valid-time/transaction-time 이원 시간축, 소급 정정, 순수 `evolve` 리플레이. Spring/Jackson import 0 (Konsist 강제) |
| `tradex-membrane` | L1 Schema Evolution Membrane | `@EventSchema` + upcaster 체인. 읽는 순간 항상 최신 버전으로 승격. 리플레이 게이트가 전 버전 fixture를 강제 |
| `tradex-saga` | L2 Deterministic Saga Engine | 엔진 자체가 이벤트소싱. 히스토리만으로 크래시 재개. **모델 체커**가 step별 결함 주입 전수 탐색(4^n)으로 불변식 위반 반례를 출력 |
| `tradex-router` | L3 Consistency Gradient Router | `X-Consistency: strong \| read-your-writes \| eventual`. HMAC 세션 토큰 + projection offset 대기 |
| `tradex-cell` | L4 Cell Fabric | 가상 노드 consistent hashing, 4단계 셀 마이그레이션(복사→캐치업→스위치→tombstone), blast radius 격리 |
| `tradex-ontology` | L5 Ubiquitous Language Registry | `ontology/*.yaml`. 미승인(DRAFT) 스키마는 기동 실패. `ontologyValidate` Gradle 태스크가 코드↔YAML diff로 빌드 게이트 |
| `tradex-runtime` | 조립 | `@EnableTradex` 하나로 전 레이어 와이어링 |
...
## tradex-* 서비스 — 레거시 기능의 TradeX 이식 (MSA)

레거시 tradexServer(커밋 `ee6ac1f`)의 인증/회원가입/멤버를 TradeX 위에 **기능별 독립 서비스**로
재구현했다. 각 서비스는 자기 TradeX 패브릭(전용 Postgres 테이블 프리픽스)을 갖고, 서로 HTTP로만
...
```bash
docker compose up -d                        # PostgreSQL (localhost:55432, tradex/tradex)
...
```bash
docker exec tradex-postgres psql -U tradex -c \
...
| 불변식 | 테스트 |
|---|---|
| asOf/asAt 차이 + 원본 보존 | `tradex-core BiTemporalAcceptanceTest`, `tradex-membrane PostgresEventStoreTest` |
| v1→v2→v3 업캐스트 체인 | `tradex-membrane UpcasterChainTest` |
| fixture 강제 (리플레이 게이트) | `tradex-membrane ReplayGateTest`, 각 `tradex-*-service`의 `*ReplayGateTest` |
| 사가 결정론적 리플레이/크래시 재개 | `tradex-saga SagaEngineTest` |
| **모델 체커 반례 (보상 타임아웃 미처리)** | `tradex-saga ModelCheckerTest` — 반례 경로가 사람이 읽게 출력된다 |
| **서비스 간 등록 사가 — 유령 계정 금지 (16경로 전수)** | `tradex-registration-service RegistrationSagaModelCheckTest` |
| RYW가 지연 주입에도 자기 쓰기 관측 | `tradex-router ConsistencyGradientAcceptanceTest` |
| 마이그레이션 전후 상태 해시 동일 | `tradex-cell CellMigrationTest` |
| blast radius 격리 | `tradex-cell CellFabricTest`, 위 curl 데모(auth-service 다운) |
...
| 의존 방향 + core 순수성 + 서비스별 도메인 격리 | `tradex-runtime ArchitectureRulesTest` (Konsist) |
