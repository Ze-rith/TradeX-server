...
>
> 이 저장소는 기존 tradexServer를 TradeX로 전환한 것이다. 레거시 코드는 커밋 `ee6ac1f`에 보존.
...
- `settings.gradle.kts` — rootProject `tradex`, 전 모듈 include
- `build.gradle.kts` — 루트(그룹/버전만)
- `buildSrc/` — convention plugins
  - `tradex.kotlin-library.gradle.kts` — JVM 21 툴체인, JUnit5, kotest
  - `tradex.spring-library.gradle.kts` — 위 + Spring BOM + spring plugin
- `tradex-core/src/main/kotlin/io/tradex/core/`
...
- `tradex-core/src/test/kotlin/` — UUIDv7 테스트, 낙관적 동시성 테스트, **bi-temporal 인수 테스트**(가격 정정 asOf/asAt/원본 보존)
- 나머지 모듈: build.gradle.kts + 빈 src (의존 방향만 선언)
- `tradex-runtime/src/test/kotlin/.../ArchitectureRulesTest.kt` — Konsist: core 순수성(Spring/Jackson import 0), 레이어 의존 방향

### M2 — Postgres 이벤트 스토어 + 스냅샷 + L1 membrane + 리플레이 게이트

- `tradex-membrane/src/main/kotlin/io/tradex/membrane/`
...
- `tradex-saga/src/main/kotlin/io/tradex/saga/`
...
- `tradex-router/src/main/kotlin/io/tradex/router/`
...
- `tradex-cell/src/main/kotlin/io/tradex/cell/`
...
- `tradex-ontology/src/main/kotlin/io/tradex/ontology/`
...
- `tradex-runtime/` — `@EnableTradex` + AutoConfiguration으로 전 레이어 조립
...
- [x] tradex-core에 Spring/Jackson import 0 (Konsist 검증)
- [x] 모델 체커 반례 데모 테스트 ≥ 1
