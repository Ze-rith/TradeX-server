...
import io.chronos.core.store.InvalidCorrectionException
...
class PostgresEventStore(
    ...
    private fun resolveCorrectionTarget(
        ...
        val (globalSeq, targetType) = insertedInBatch[targetId]
            ?: jdbc.sql("SELECT global_seq, event_type FROM event_store WHERE event_id = :eventId AND aggregate_id = :aggregateId")
    ...
    override fun readStream(aggregateId: AggregateId, afterSeqNo: Long): List<EventRecord<DomainEvent>> =
        jdbc.sql("SELECT * FROM event_store WHERE aggregate_id = :aggregateId AND seq_no > :afterSeqNo ORDER BY seq_no")
            .param("aggregateId", aggregateId.value)
    ...
    override fun readStreamAsAt(aggregateId: AggregateId, transactionTime: Instant): List<EventRecord<DomainEvent>> =
        jdbc.sql(
            "SELECT * FROM event_store WHERE aggregate_id = :aggregateId AND transaction_time <= :asAt ORDER BY seq_no",
        )
            .param("aggregateId", aggregateId.value)
    ...
    override fun readAll(afterGlobalSeq: Long, limit: Int): List<EventRecord<DomainEvent>> =
        jdbc.sql("SELECT * FROM event_store WHERE global_seq > :after ORDER BY global_seq LIMIT :limit")
    ...
    override fun lastGlobalSeq(): Long =
        jdbc.sql("SELECT coalesce(max(global_seq), 0) FROM event_store")
            .query(Long::class.java)
            .single()

    private fun currentSeqNo(aggregateId: AggregateId): Long =
        jdbc.sql("SELECT coalesce(max(seq_no), 0) FROM event_store WHERE aggregate_id = :aggregateId")
            .param("aggregateId", aggregateId.value)
            .query(Long::class.java)
            .single()
